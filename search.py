...
def call_remote_control(args: list[str]) -> None:
    res = subprocess.run(["kitty", "@", *args], capture_output=True, text=True)
    if res.returncode != 0:
        if "Could not resize" in res.stderr:
            return
        with open("/tmp/kitty-search-debug.log", "a") as f:
            f.write(f"Command failed: {args}\n")
...
try:
    _kitty_dir = Path(__file__).parent.absolute()
except NameError:
    import os
    _kitty_dir = Path(os.environ.get("KITTY_CONFIG_DIRECTORY", str(Path.home() / ".config/kitty")))
SCROLLMARK_FILE = _kitty_dir / "scroll_mark.py"
...
def main(args: list[str]) -> None:
    ...
    call_remote_control(
        ["resize-window", "--self", "--axis=vertical", "--increment", "3"]
    )
    ...
    if len(args) > 1 and args[1].isdigit():
        ...
        import os
        env_win_id = os.environ.get("KITTY_WINDOW_ID")
        if env_win_id and env_win_id.isdigit():
            window_id = int(env_win_id)
        else:
